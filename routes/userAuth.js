...
// Validate JWT_KEY (optional if removing JWT)
if (!process.env.JWT_KEY) {
  console.warn("JWT_KEY environment variable is not set");
}
...
// Render auth success page (optional, can be removed if not used)
router.get("/auth-success", (req, res) => {
  res.redirect("/client-page"); // Fallback redirect, ideally not reached
});
...
router.post("/signup/email", async (req, res) => {
  ...
  try {
    ...
    // Log in the user to set the session
...
router.post("/login", (req, res, next) => {
  passport.authenticate("local", (err, user, info) => {
    ...
    if (!user) {
      console.log("Invalid login attempt:", req.body.email);
...
router.get("/logout", (req, res) => {
  req.logout((error) => {
    ...
    req.session.destroy((err) => {
      ...
      res.send("Thank you for visiting Kings Collections");
    });
  });
});
